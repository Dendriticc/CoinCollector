...
public class SuperCoinAgent : Agent
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.tag == "LAVA" && rigidbody.velocity.y <= -1f)
        {
            AddReward(-0.5f);
        }
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        if (rigidbody != null)
        {
            ...
            rigidbody.AddForceAtPosition(forceDirection * 4f, transform.position, ForceMode.Impulse);
        }
    }

    public float CalculateBoxReward()
    {
        float startingPos = castleArea.startingBlockPositionZ;
        float endingPos = castleArea.block.transform.position.z;
        return startingPos > endingPos ?
        -Mathf.Abs(Mathf.InverseLerp(startingPos, 0, endingPos)) : 1 - Mathf.Abs(Mathf.InverseLerp(-10, startingPos, endingPos));
    }
}